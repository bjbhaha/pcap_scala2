...
import static org.apache.hadoop.hive.serde2.thrift.test.PropValueUnion.doubleValue;

public class pcaptest6 {
...
//                });
        ...
        JavaPairRDD<Double, Long> rtsRDD = javaRDD.mapToPair(new PairFunction<Tuple2<IntWritable,BytesWritable>,Double,Long>(){
            public Tuple2<Double,Long> call(Tuple2<IntWritable,BytesWritable> tt) throws Exception{
                ...
                String TSstr = str1+str2;//TS_usec.toString()+TS_micros.toString();
                long stp = Long.parseLong(TSstr);//μs (10).(6)
                SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                long milliSec = (long)Math.floor(stp/Math.pow(10,3));
                String sd = sdf.format(new Date(milliSec));      // 时间戳转换成时间
                //System.out.println("格式化结果：" + sd);
                Double timestp = (double)stp/(Math.pow(10,5));//100ms
                Double roughT = Math.floor(timestp);//roughT=[timestp]
                Long size = (long) tt._2.getLength();//Byte
                return new Tuple2<Double,Long>(roughT,size);
            }
        });

        //ysh
        /*JavaPairRDD<Double,Long> tsRDD = sc.parallelizePairs(tslist);
        JavaPairRDD<Double,Long> tsRDD1 = tsRDD.reduceByKey((a,b) -> a+b);
        JavaPairRDD<Double,Long> tsRDDsorted = tsRDD1.sortByKey(true);
        //statistic based on relative timestp
        //actual timestamp for vertical ordinate
        List<Tuple2<Double,Long>> startlist = tsRDDsorted.take(1);
        //Double start = (startlist.get(0)._1)/10;//s
        List<Tuple2<Double,Long>> endlist = tsRDDsorted.take(tslist.size()-1);
        //Double end = (endlist.get(0)._1)/10;//s
        */
        ...
        resultList = rtsRDDsorted.collect();//
        for(Tuple2<Double,Long>tt:resultList){
            String str = new String();
            str = String.format("%.0f",(double)(tt._1));
            System.out.println("interval start at "+str+" size is "+tt._2+"\n");
        }
